...
final class CliAttachmentRefCapturerTest extends TestCase {
	...
	public function test_capture_records_logo_attachment_with_meta_and_files(): void {
		...
		// Logo files list should include the original + each size variant.
		...
		// option_ref_to_file maps both option keys to their file paths.
	...
	public function test_capture_skips_non_attachment_posts(): void {
		// If an option_keys_attachment_refs value references a post
		// that isn't an attachment (designer somehow pointed it at a
		// page, or the option was misconfigured), skip it silently.
		$c = new AttachmentRefCapturer(
	...
	public function test_capture_dedups_by_file_when_two_options_reference_same_attachment(): void {
		// site_logo and custom_logo often point at the same attachment.
		// Both should end up in option_ref_to_file but by_file has only
		// one entry.
		$options = array(
			'site_logo'   => '42',
			'custom_logo' => '42',
